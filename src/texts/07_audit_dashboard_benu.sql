...
select distinct("shop")	
    , count(distinct("slug")) over () as "Produktu_celkem" --"count_clean"	
...
select 	
    case	
        when count("slug") is null then 0	
        else count(distinct("slug"))	
...
select "shop"	
    ...
    , row_number() over (partition by "incr"."itemId", "incr"."slug" order by "datum" desc) as "row_num"	
from	
(select *	
    , (lag("refCena_dle_shopu") over (partition by "itemId", "slug" order by "datum" asc)) as "refCena_puv"	
    , case 	
        when (lag("refCena_dle_shopu") ignore nulls over (partition by "itemId", "slug" order by "datum" asc))  < "refCena_dle_shopu" then 'incr'	
        when (lag("refCena_dle_shopu") ignore nulls over (partition by "itemId", "slug" order by "datum" asc))  > "refCena_dle_shopu" then 'decr'	
        else ''	
      end as "incr"	
from "shop_30d"	
...
on "incr"."itemId" = "disc"."itemId" and "incr"."slug" = "disc"."slug"
