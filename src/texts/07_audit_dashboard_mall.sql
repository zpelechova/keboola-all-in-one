...
select "diff".*
    ...
    , "partner"."PARTNER" as "patnersky_prodej"
...
left join (select "ITEM_ID"
           ...
           , left("CREATED",10) as "CREATED"
            , max(left("CREATED",10)) over () as "date"
           from "mall_cz_xml"
...
left join (select "ITEM_ID"
           ...
           , left("CREATED",10) as "CREATED"
            , max(left("CREATED",10)) over () as "date"
           from "mall_cz_xml"
           qualify left("CREATED",10) >= dateadd("day",-30,"date")) "partner"
on "incr"."itemId" = "partner"."ITEM_ID" and "incr"."datum" = "partner"."CREATED"
