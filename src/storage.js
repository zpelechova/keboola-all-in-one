...
export async function getOrCreateTable(shopName) {
    ...
    const getUrl = "https://connection.eu-central-1.keboola.com/v2/storage/tables?include=buckets,columns,metadata,columnMetadata";
    const getMethod = "GET";
    const getHeaders = {
    ...
    const getStorageData = JSON.parse(getBody).find((i) => i.id === `in.c-black-friday.${shopName}`);
    if (getStorageData) {
        console.log(`The table ${shopName} already exists, returning its information.`);
        return getStorageData;
    };

    // Otherwise, create

    console.log(`The table ${shopName} doesn't exist, I am going to create it now.`);
    ...
    const postUrl = 'https://connection.eu-central-1.keboola.com/v2/storage/buckets/in.c-black-friday/tables-async'
